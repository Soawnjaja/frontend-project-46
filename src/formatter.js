...
// работаем с полученным массивом разниц и формируем текстовое дерево
// используем в главном модуле
...
const stringify = (data, depth) => {
  ...
  const lines = arr.map(
    ([key, val]) =>
      `${indention(depth + 1)}  ${key}: ${stringify(val, depth + 1)}`
  );
  const result = ["{", ...lines, `${indention(depth)}  }`].join("\n");
  return result;
};
const data = makeDiff(tree1, tree2);
